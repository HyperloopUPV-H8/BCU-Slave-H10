...
ThreePhaseSystem::ThreePhaseSystem(float u, float v, float w)
    : u(u), v(v), w(w) {}

TwoPhaseSystem::TwoPhaseSystem(float alpha, float beta)
    : alpha(alpha), beta(beta), zero(0.0f) {}

RotatingTwoPhaseSystem::RotatingTwoPhaseSystem(float d, float q)
    : d(d), q(q), zero(0.0f) {}
...
RotatingTwoPhaseSystem TwoPhaseSystem::park_transform(float electrical_angle) {
